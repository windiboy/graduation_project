...
protected:
    ...
    ros::NodeHandle nh_;
    //定义action服务端
    actionlib::ActionServer<control_msgs::GripperCommandAction>  as_;
    //定义action服务端目标控制句柄
    actionlib::ServerGoalHandle<control_msgs::GripperCommandAction> goal_handle_;
    //用来反馈action目标的执行情况，客户端由此可以得知服务端是否执行成功了
    control_msgs::GripperCommandAction result_;
...
public:
    ...
    void goalCB( actionlib::ServerGoalHandle<control_msgs::GripperCommandAction> gh)
    {
        //疑点1：什么要将形参复制一份？
        //因为我们要修改这个参数，但又不#include "../include/MotorDriver.h"能直接改形参所指向的那个目标对象，那样做会破坏原始数据，是编程所不推荐的，这里复制一个副本，目的就是要修改这个数据而不破坏原始数据。
        actionlib::ActionServer<control_msgs::GripperCommandAction>::Goal goal = *gh.getGoal();    //make a copy that we can modify
        ...
        // result_.command.reached_goal = true;
